...
export default function ProductDetails() {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        const response = await axios.get(`https://deone.org/api/endpoints/get_one_product_with_images.php?id_producto=${id}`);
  ...
  const handleAgregarAlCarrito = () => {
    ...
    if (productoEnCarrito) {
      ...
      carrito.push({
        ...
        cantidad: cantidad,
        imagen: producto.imagenes[0]?.nombre || imgnotfound 
      });
    }
  ...
  const handlePedirWhatsApp = () => {
    const mensaje = `Hola, quiero pedir el producto: ${producto.nombre} (Cantidad: ${cantidad}). Total: S/${producto.precio * cantidad}. Puedes ver más detalles aquí: http://localhost:5173/productos/${producto.id_producto}`;
    const telefonoWhatsApp = "51986262416";  
            ...
            <div className="lg:w-1/2 w-full">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="bg-rose-500 text-white py-3 px-6 rounded-lg hover:bg-rose-600 transition duration-300"
                >
                  Agregar al carrito
                </button>
                <button
                  onClick={handlePedirWhatsApp}
                  className="border border-rose-500 text-rose-500 py-3 px-6 rounded-lg hover:bg-rose-600 hover:text-white transition duration-300"
                >
                  Pedir al WhatsApp
                </button>
              </div>
